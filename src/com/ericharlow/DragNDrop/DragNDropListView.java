...
public class DragNDropListView extends ListView {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if(mCurrentPosition > 0) {
					...
					if (mCurrentPosition == getFirstVisiblePosition()) {
						try {
							this.smoothScrollToPosition (mCurrentPosition-1); // android 2.2
					...
					if (mCurrentPosition != mItemPosition) {
						...
						mDragListener.onStopDrag(getChildAt(mItemPosition));	
						mItemPosition = mCurrentPosition;
						mStartPosition = mItemPosition;
					}
				}
				break;
			...
			default:
				//if(mDragView!=null)mDragView.postInvalidate();
				mDragMode = false;
